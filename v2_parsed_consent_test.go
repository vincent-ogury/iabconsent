...
func (p *V2ParsedConsentSuite) TestPurposeAllowedForConsent(c *check.C) {
	var tcs = []struct {
		purposes []int
		consent  map[int]bool
	}{
	...
	for _, tc := range tcs {
		...
		for i, p := range tc.purposes {
			c.Check(pc.PurposeAllowedForConsent(p), check.Equals, tc.consent[i+1])
		}

	}
}

func (v *V2ParsedConsentSuite) TestVendorAllowedForConsent(c *check.C) {
	var tcs = []struct {
		vendor  int
		isRange bool
	...
	for _, tc := range tcs {
		...
		c.Check(pc.VendorAllowedForConsent(tc.vendor), check.Equals, tc.exp)
	}
}
